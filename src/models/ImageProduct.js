...
class ImageProduct {
    ...
    static async create(imgProductData) {
        const { product_id, image_url, is_default = false } = imgProductData; // Gán mặc định cho is_default nếu không có
        ...
        `;
        try {
            const [result] = await db.execute(query, [product_id, image_url, is_default]);
    ...
    static async delete(imgProductId) {
        ...
        await db.execute(query, [imgProductId]);
        return true;
    }
    ...
    static async setDefault(imgProductId) {
        try {
            ...
            const [image] = await db.execute('SELECT product_id FROM image_product WHERE id = ?', [imgProductId]);
            if (image.length === 0) {
            ...
            await db.execute('UPDATE image_product SET is_default = false WHERE product_id = ?', [productId]);
            await db.execute('UPDATE image_product SET is_default = true WHERE id = ?', [imgProductId]);
